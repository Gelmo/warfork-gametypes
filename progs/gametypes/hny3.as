...
const int HOONII_ROUNDSTATE_GAMEFINISHED = 13;
        ...
        switch ( this.state )
        {
            ...
            */
            ...
            case HOONII_ROUNDSTATE_ROUNDFINISHED:
            {
                ...
                @ent0 = @G_GetClient( 0 ).getEnt();
                @ent1 = @G_GetClient( 1 ).getEnt();
                if(ent0.client.stats.score >= scoreLimit.integer)
                {
                    if(abs(ent0.client.stats.score - ent1.client.stats.score) >= 2)
                    {
                        winnerfound = true;
                    }
                }

                if(ent1.client.stats.score >= scoreLimit.integer)
                {
                    if(abs(ent1.client.stats.score - ent0.client.stats.score) >= 2)
                    {
                        winnerfound = true;
                    }
                }
                ...
                if(winnerfound)
                {
                    if(G_GetTeam(TEAM_ALPHA).ent(0).client.stats.score > G_GetTeam(TEAM_BETA).ent(0).client.stats.score)
                    {
                        G_PrintMsg(null,G_GetTeam(TEAM_ALPHA).ent(0).client.name+" has won!\n");
                        this.newRoundState(HOONII_ROUNDSTATE_GAMEFINISHED);
                    }
                    if(G_GetTeam(TEAM_BETA).ent(0).client.stats.score > G_GetTeam(TEAM_ALPHA).ent(0).client.stats.score)
                    {
                        G_PrintMsg(null,G_GetTeam(TEAM_BETA).ent(0).client.name+" has won!\n");
                        this.newRoundState(HOONII_ROUNDSTATE_GAMEFINISHED);
                    }
                }
...
void GT_InitGametype()
{
    ...
    G_RegisterCallvote( "hoony_randspawn", "1 or 0", "bool", "Enables or disables random spawn selection" );
