	...
	for(t = TEAM_ALPHA; t < GS_MAX_TEAMS; t++) {
		...
		for(i = 0; @team.ent(i) != null; i++) {
			...
			if(gametype.isInstagib) {
				...
				// "AVATAR Name Clan Score Frags Dfrst Ping C R"
				entry = "&p " + playerID + " " + playerID + " " + ent.client.clanName + " "
...
void GT_InitGametype() {
	...
	if(gametype.isInstagib) {
		...
		G_ConfigString(CS_SCB_PLAYERTAB_LAYOUT, "%a l1 %n 112 %s 52 %i 52 %i 52 %i 52 %l 48 " + "%p 18 " + "%p 18");
		G_ConfigString(CS_SCB_PLAYERTAB_TITLES, "AVATAR Name Clan Score Frags Dfrst Ping " + "C " + " R");
	}
