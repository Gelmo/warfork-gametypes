...
Cvar g_wipeout_delay( "g_wipeout_delay", "5", 0 );
Cvar g_wipeout_teamdelay( "g_wipeout_teamdelay", "0", 0 );
...
uint[] respawn_delay( maxClients );
uint[] respawn_delay_team( GS_MAX_TEAMS );
int[] respawn_time( maxClients );
int[] respawn_time_remaining( maxClients );

int last_alpha_remaining, last_beta_remaining;
Entity@ last_alphaSurvivor, last_betaSurvivor;

void WIPEOUT_setRespawnDelay( Client@ client, bool from_spec = false )
{
	uint delay_increment = int( g_wipeout_delay.value * 1000 );
	uint delay = 0;
	if ( g_wipeout_teamdelay.boolean )
	{
		delay = respawn_delay_team[ client.team ];
		if ( !from_spec )
			delay += delay_increment;
		respawn_delay_team[ client.team ] = delay;
	}
	else
	{
		if ( from_spec )
		...
		{
			delay = respawn_delay[ client.playerNum ] + delay_increment;
		}
		respawn_delay[ client.playerNum ] = delay;
		if ( respawn_delay_team[ client.team ] < delay )
			respawn_delay_team[ client.team ] = delay;
	}
	respawn_time[ client.playerNum ] = levelTime + delay;
	respawn_time_remaining[ client.playerNum ] = 1 + delay / 1000;
}

void WIPEOUT_resetDelays()
{
	int i;
	for ( i = TEAM_PLAYERS; i < GS_MAX_TEAMS; i++ )
	{
		respawn_delay_team[ i ] = 0;
	}
	...
	for ( i = 0; i < maxClients; i++ )
	{
		respawn_delay[ i ] = 0;
		respawn_time[ i ] = -1;
		respawn_time_remaining[ i ] = -1;
	}
}

void G_PrintMsgTeam( int team_id, String& msg )
{
	Team@ team = @G_GetTeam( team_id );
	Entity@ ent;
...
class cCARound
{
	...
	void think()
	{
		...
		{		
			...
			for ( int i = TEAM_ALPHA; i < GS_MAX_TEAMS; i++ )
			{
				...
				{
					@ent = @team.ent( j );
					int playerNum = ent.client.playerNum;
					if ( !ent.isGhosting() )
					{
						count++;
					}
		...
		if ( this.state == CA_ROUNDSTATE_PREROUND || this.state == CA_ROUNDSTATE_ROUND )
		{
			Entity@ alphaSurvivor, betaSurvivor;
			int alphaCount = 0, betaCount = 0;
			int alphaRemainingSeconds = -1, betaRemainingSeconds = -1;
			...
			for ( int i = TEAM_ALPHA; i < GS_MAX_TEAMS; i++ )
			{
				...
				for ( int j = 0; @team.ent( j ) != null; j++ )
				{
					...
					int playerNum = ent.client.playerNum;
					if ( ent.isGhosting() && respawn_time[ playerNum ] > 0 )
					{
						if ( uint( respawn_time[ playerNum ] ) <= levelTime )
						{
							ent.client.respawn( false );
							respawn_time[ playerNum ] = -1;
							respawn_time_remaining[ playerNum ] = -1;
							G_CenterPrintMsg( ent, "\n" ); // clear message
						} else {
							int remainingSeconds = 1 + ( respawn_time[ playerNum ] - levelTime ) / 1000;

							if ( i == TEAM_ALPHA )
							{
								if ( alphaRemainingSeconds == -1 || remainingSeconds < alphaRemainingSeconds )
									alphaRemainingSeconds = remainingSeconds;
							} else {
								if ( betaRemainingSeconds == -1 || remainingSeconds < betaRemainingSeconds )
									betaRemainingSeconds = remainingSeconds;
							}

							if ( remainingSeconds != respawn_time_remaining[ playerNum ] )
							{
								respawn_time_remaining[ playerNum ] = remainingSeconds;
								if ( remainingSeconds > 3 )
								{
									G_CenterPrintMsg( ent, S_COLOR_GREY + "Respawning in " + remainingSeconds + " seconds\n" );
								}
								else if ( remainingSeconds > 1 )
								{
									G_LocalSound( ent.client, CHAN_AUTO, G_SoundIndex( "sounds/bomb/bombtimer" ) );
									G_CenterPrintMsg( ent, S_COLOR_WHITE + "Respawning in " + remainingSeconds + " seconds" + "\n" );
								}
								else
								{
									G_LocalSound( ent.client, CHAN_AUTO, G_SoundIndex( "sounds/bomb/bombtimer" ) );
									G_CenterPrintMsg( ent, S_COLOR_WHITE + "Respawning in " + remainingSeconds + " second" + "\n" );
								}
							}
						}
					}
			...
			if ( alphaCount == 1 && alphaRemainingSeconds != -1 )
			{
				if ( last_alpha_remaining != alphaRemainingSeconds )
				{
					last_alpha_remaining = alphaRemainingSeconds;
					G_CenterPrintMsg( alphaSurvivor, S_COLOR_GREY + "Support in " + alphaRemainingSeconds + " seconds\n" );
				}
				@last_alphaSurvivor = @alphaSurvivor;
			} 
			else
			{
				if ( last_alpha_remaining != -1 )
				{
					G_CenterPrintMsg( last_alphaSurvivor, "\n" ); // clear message
					last_alpha_remaining = -1;
				}
			}

			if ( betaCount == 1 && betaRemainingSeconds != -1 )
			{
				if ( last_beta_remaining != betaRemainingSeconds )
				{
					last_beta_remaining = betaRemainingSeconds;
					G_CenterPrintMsg( betaSurvivor, S_COLOR_GREY + "Support in " + betaRemainingSeconds + " seconds\n" );
				}
				@last_betaSurvivor = @betaSurvivor;
			} 
			else
			{
				if ( last_beta_remaining != -1 )
				{
					G_CenterPrintMsg( last_betaSurvivor, "\n" ); // clear message
					last_beta_remaining = -1;
				}
			}
		}
	}
	...
	void playerKilled( Entity @target, Entity @attacker, Entity @inflictor )
	{
		...
		if ( this.state == CA_ROUNDSTATE_PREROUND || this.state == CA_ROUNDSTATE_ROUND )
		{
			...
			{
				WIPEOUT_setRespawnDelay( target.client, false );
			}
		}
		...
		if ( @target != null && @target.client != null && @attacker != null && @attacker.client != null )
		{
			...
			int bestAttackerTime = -1, bestTargetTime = -1;
			Entity@ survivor;
			...
			for ( int j = 0; @team.ent( j ) != null; j++ )
			{
				...
				else if ( bestAttackerTime == -1 || respawn_time_remaining[ ent.playerNum ] < bestAttackerTime )
				{
					bestAttackerTime = respawn_time_remaining[ ent.playerNum ] - 1;
				}
			}
			...
			for ( int j = 0; @team.ent( j ) != null; j++ )
			{
				...
				else if ( bestTargetTime == -1 || respawn_time_remaining[ ent.playerNum ] < bestTargetTime )
				{
					bestTargetTime = respawn_time_remaining[ ent.playerNum ] - 1;
				}
			}
			...
			if ( attackerCount == 1 && targetCount == 1 )
			{
				...
				survivor.client.addAward( S_COLOR_ORANGE + "1v1! Good Luck!" );
			}
			else if ( attackerCount == 1 && targetCount > 1 )
			{
				attacker.client.addAward( "1v" + targetCount + "! You're on your own!" );
				G_PrintMsgTeam( attacker.team, "1v" + targetCount + "! " + attacker.client.name + " is on their own!\n" );
			...
			{
				survivor.client.addAward( "1v" + attackerCount + "! You're on your own!" );
				G_PrintMsgTeam( survivor.team, "1v" + attackerCount + "! " + survivor.client.name + " is on their own!\n" );
...
bool GT_Command( Client @client, const String &in cmdString, const String &in argsString, int argc )
{
	...
	else if ( cmdString == "callvotevalidate" )
	{
		...
		if ( votename == "teamdelay" )
		{
			...
			if ( value == g_wipeout_teamdelay.integer )
			{
				client.printMessage( "Teamdelay is already " + ( voteArg == 1 ? "" : "dis" ) + "allowed\n" );
				return false;
			}
		...
		if ( votename == "delay" )
		{
			...
			if ( value <= 0 )
			{
				client.printMessage( "Callvote " + votename + " expects a positive number as argument\n" );
				return false;
			}

			if ( value == g_wipeout_delay.value )
			{
				client.printMessage( "Delay is already " + value + "\n" );
				return false;
			}
	...
	else if ( cmdString == "callvotepassed" )
	{
		...
		if ( votename == "teamdelay" )
		{
			if ( argsString.getToken( 1 ).toInt() > 0 )
				g_wipeout_teamdelay.set( 1 );
			else
				g_wipeout_teamdelay.set( 0 );
		}

		if ( votename == "delay" )
		{
			g_wipeout_delay.set( argsString.getToken( 1 ).toFloat() );
		}
...
String @GT_ScoreboardMessage( uint maxlen )
{
	...
	for ( t = TEAM_ALPHA; t < GS_MAX_TEAMS; t++ )
	{
		...
		for ( i = 0; @team.ent( i ) != null; i++ )
		{
			...
			int player_respawn_delay = respawn_time_remaining[ ent.playerNum ];
			String player_respawn_delay_str = "\u00A0";
			if ( ( caRound.state == CA_ROUNDSTATE_PREROUND || caRound.state == CA_ROUNDSTATE_ROUND ) && ent.isGhosting() && player_respawn_delay > 0 )
				player_respawn_delay_str = player_respawn_delay;
...
{
	if ( caRound.state == CA_ROUNDSTATE_PREROUND || caRound.state == CA_ROUNDSTATE_ROUND )
	{
		if ( new_team != TEAM_SPECTATOR && old_team != new_team )
		{
			WIPEOUT_setRespawnDelay( ent.client, true );
		}
	}
...
void GT_InitGametype()
{
	...
	if ( !G_FileExists( "configs/server/gametypes/" + gametype.name + ".cfg" ) )
	{
		...
		config = "// '" + gametype.title + "' gametype configuration file\n"
				 ...
				 + "\n// gametype settings\n"
				 + "set g_wipeout_teamdelay \"0\"\n"
				 + "set g_wipeout_delay \"5\"\n"
				 + "\n// classes settings\n"
	...
	G_RegisterCallvote( "teamdelay", "<1 or 0>", "bool", "If enabled, respawn delay increases for whole team" );
	G_RegisterCallvote( "delay", "<seconds>", "integer", "Amount of seconds to add to respawn delay" );
